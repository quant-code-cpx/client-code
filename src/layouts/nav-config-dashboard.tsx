...
import { knowledgeMajorMetadata } from 'src/sections/knowledge-base/content/knowledge-major-metadata';
...
export function createNavData(agentEnabled = CONFIG.agentEnabled, role?: UserRole): NavItem[] {
  return [
      ...
      : []),
    {
      ...
      icon: <Iconify icon="solar:notebook-bookmark-bold" width={24} />,
      children: knowledgeMajorMetadata.map((major) => ({
        title: major.title,
        path: paths.knowledge.major(major.slug),
        icon: <Iconify icon={major.icon} width={24} />,
      })),
    },
    {
      title: '预警监控',
