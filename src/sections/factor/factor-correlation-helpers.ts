...
/** 生成 CSV 内容（包含 meta、矩阵、nMatrix、coverage） */
export const formatCorrelationCsv = (result: FactorCorrelationResult): string => {
  const { factors, factorLabels, matrix, nMatrix, coverage, meta, tradeDate, method } = result;
  const labels = factorLabels?.length === factors.length ? factorLabels : factors;
  const lines: string[] = [];

  lines.push('# 因子相关性导出');
  lines.push(`# tradeDate,${tradeDate}`);
  lines.push(`# method,${method}`);
  if (meta) {
    lines.push(`# universe,${csvEscape(meta.universe ?? '')}`);
    lines.push(`# computedAt,${csvEscape(meta.computedAt ?? '')}`);
    lines.push(`# matrixMode,${meta.matrixMode ?? ''}`);
    lines.push(`# minSampleForCorr,${meta.minSampleForCorr ?? ''}`);
    lines.push(`# rankTiesMethod,${meta.rankTiesMethod ?? ''}`);
  }
  lines.push('');

  lines.push('# 相关系数矩阵（null 表示样本不足或常数序列）');
  lines.push(['factor', ...factors.map(csvEscape)].join(','));
  matrix.forEach((row, i) => {
    const cells = row.map((v) => (v === null || v === undefined ? '' : v.toFixed(4)));
    lines.push([csvEscape(`${factors[i]} · ${labels[i]}`), ...cells].join(','));
  });
  lines.push('');

  if (nMatrix && nMatrix.length === factors.length) {
    lines.push('# 有效样本数矩阵（pairwise）');
    lines.push(['factor', ...factors.map(csvEscape)].join(','));
    nMatrix.forEach((row, i) => {
      lines.push([csvEscape(factors[i]), ...row.map((v) => String(v ?? ''))].join(','));
    });
    lines.push('');
  }

  if (coverage && coverage.length === factors.length) {
    lines.push('# 单因子覆盖率');
    lines.push('factor,label,coverage');
    coverage.forEach((c, i) => {
      lines.push([csvEscape(factors[i]), csvEscape(labels[i]), c.toFixed(4)].join(','));
    });
  }
...
/** 触发浏览器下载 CSV */
export const downloadCorrelationCsv = (result: FactorCorrelationResult): void => {
  const content = formatCorrelationCsv(result);
  const blob = new Blob([`\uFEFF${content}`], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `factor-correlation-${result.tradeDate}-${result.method}.csv`;
