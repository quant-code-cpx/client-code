...
type Counter = { name: string; up: number; down: number };
...
function countByIndustry(items: LimitListItem[]): Counter[] {
  ...
  items.forEach((it) => {
    ...
    if (!name) return;
    const cur = map.get(name) ?? { name, up: 0, down: 0 };
    if (it.limitType === 'UP') cur.up += 1;
    else cur.down += 1;
    map.set(name, cur);
  });
...
export function AlertLimitMainstreamBar({ items, topN = 10, onIndustryClick }: Props) {
  ...
  const max = data.reduce((m, d) => Math.max(m, d.up + d.down), 1);
        ...
        {!hasIndustry ? (
          ...
          <Stack spacing={1}>
            {data.map((row) => {
              ...
              const downWidth = (row.down / max) * 100;
              return (
                <Box
                  key={row.name}
                  sx={{
