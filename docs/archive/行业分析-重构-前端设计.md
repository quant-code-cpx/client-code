...
> 状态：📐 设计中（阶段一）
> 归档说明（2026-08-09）：已被[行业分析 v3](行业分析-重构v3-前端设计.md)取代；当前路由与模块结构以 v3 为准。
> 创建日期：2026-04-26
...
5. **操作区对齐**：行业轮动 Tab 与市场热力图 Tab 的刷新、筛选、帮助入口保持一致。
...
- **数据**：保持现状 6 个端点。
- **交互变更**：
  - 移除内嵌 H4、TextField 日期；接受外部 `tradeDate` prop。
...
- 精简 Tab 1 工具栏，保留与外层工作台一致的刷新和帮助入口。
...
| `rotation/view/industry-rotation-view.tsx`                                         | 改造 | 接受外部 props；`embedded` 模式下去掉 `DashboardContent` + 内嵌 H4 + TextField；新增 `contentType` 与 `focusedSector` 高亮 |
...
| `src/api/industry-dict.ts`                                                         | 新增 | 封装 `fetchIndustryDictMapping()`；对外提供 `IndustryDictMappingItem` / `IndustryDictMappingResponse` 等类型               |
