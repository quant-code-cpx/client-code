...
> 路由：`/factor/screening`
> 关联后端：`/api/factor/library`、`/api/factor/screening`、`/api/factor/backtest/*`、`/api/factor/optimization`、`/api/watchlist/stocks/batch`、`/api/export/*`
...
- **3 分钟动作**：搭条件 → 看命中概览 → 解释为什么入选 → 加入自选股 / 保存为策略 / 发起回测 / 建条件订阅 / 导出结果。
- **边界声明**：
...
| 9   | 导出当前筛选结果             | 离线复盘、发给同事、接入研究笔记                   | 需修正导出契约                             |
...
- [x] **Q4**：闭环动作优先级（高→低）：① 加入自选股 → ② 导出 → ③ 保存策略 → ④ 快速回测 → ⑤ 创建订阅。前 2 项依赖已有后端能力，阶段二必做；后 3 项依赖后端契约修复，阶段二接通已就绪部分，未就绪的按钮置灰并提示“等待后端”。
...
| 结果表格              | `src/sections/factor/factor-screening-table.tsx`                                     | `FactorScreeningResult`                                        | 展示排名、代码、名称、行业、动态因子列；分页；导出按钮                  |
...
| 导出能力（相邻）      | `src/components/export-button`、`src/api/export.ts`、`server-code/src/apps/export/*` | 前端 `POST /api/export`；后端 `POST /api/export/factor-values` | 存在契约错位，当前表格导出无法代表“当前筛选结果”                        |

### 2.2 不足之处（按严重性排序）

| #    | 问题                                                                                                                                                                                              | 影响                                                             | 触发场景                  |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------- |
| P0-1 | **导出契约错位**：表格调用 `<ExportButton source="factor_values" />`，没有传 `conditions/tradeDate/universe/sort`；前端导出请求是 `POST /api/export`，后端实际是 `POST /api/export/factor-values` | 导出的不是当前筛选结果，甚至可能直接失败                         | 用户点击“导出”            |
...
| P1-8 | **后端每次筛选逐因子拉取全量截面再内存过滤**                                                                                                                                                      | 因子多、股票池大、分页/导出频繁时成本高                          | 5+ 因子、全市场、多人并发 |
...
| 对应问题 | 应对策略                                                                                       | 取舍说明                                               |
| -------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------- | -------------------- |
| P0-1     | 新增“导出当前筛选结果”专用契约：前端传完整 `FactorScreeningDto`，后端导出与表格同一套筛选逻辑  | 不复用 `factor-values`，避免语义混淆                   |
...
| P1-6     | 结果表增加批量选择，顶部 Action Bar：加入自选股、保存策略、发起回测、创建订阅、导出            | 先接已有 `watchlist/stocks/batch`；其他依赖后端修契约  |
| P1-7     | 阶段二先修前后端类型：`save-as-strategy` 响应、`attribution` 入参/响应、回测状态字段           | 不修契约不把回测面板搬进选股页                         |
| P1-8     | 后端增加筛选请求 hash / cache / async job：分页、导出、回测复用同一次筛选快照                  | 大数据量时必要；小数据量仍可同步返回                   |
...
- **交互**：
  ...
  - 导出：调用新的 factor screening export 契约。
...
- **请求一致性**：分页、导出、回测、保存策略都引用同一个条件快照；如果条件变更，结果区标记“已过期，请重新运行”。
- **跨模块联动**：
...
│ └───────────────────────────────────────────────────────────────────────┘ │
│ Selected 12: [加入自选股] [保存策略] [快速回测] [创建订阅] [导出]           │
└───────────────────────────────────────────────────────────────────────────┘
```
...
- **条件快照**：每次 Run 生成 `requestHash`，结果、分页、导出、回测、保存策略都绑定这一版快照。
...
- **前后端契约修复优先级**：先修导出、类型 null、save-as-strategy/attribution 字段，再做大组件搬迁。
...
| 导出接口重构牵涉全局 ExportButton     | 影响其他模块导出   | 为因子选股单独封装 `FactorScreeningExportButton` |
...
| #    | 字段 / 端点                                                                                    | 说明                                                                                                   |
| ---- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------- |
| BE-1 | `POST /api/export/factor-screening` 或统一 `POST /api/export` 支持 `source='factor_screening'` | 入参复用 `FactorScreeningDto + columns + format`，导出必须与当前筛选结果一致                           |
...
| BE-6 | 返回 `requestId/requestHash`                                                                   | 分页、导出、回测、保存策略复用同一版条件快照，避免条件变更后串数据                                     |
...
| BE-17 | `POST /api/factor/screening/audit/list`                             | 查询用户近期运行条件、导出、保存策略、订阅创建等操作日志 |
...
- [ ] 条件校验、URL 持久化、导出参数、null 字段渲染都有单元测试。

### 6.4 验收数据样例

| 场景           | 数据                                            | 预期                                                |
| -------------- | ----------------------------------------------- | --------------------------------------------------- |
| 正常交易日     | `20260429`，沪深300，3 条有效条件，命中 42      | KPI、表格、漏斗、导出、批量动作均正常               |
...
| 导出           | 当前条件命中 42，导出 xlsx                      | 文件只包含 42 条当前结果与动态因子列                |
...
- `docs/design/因子库-重构-前端设计.md` 聚焦因子目录与质量指标。
