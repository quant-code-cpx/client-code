...
| 11 | 保存草稿、测试、发布三步；采用配置版本与原子 registry swap | 写库成功但 reload 失败时保留旧活动版本 |
...
- **职责**：解释“为什么会选/不会选某模型”。
- **数据**：路由候选、capability/dataClass/health/cost/priority 排除原因，P50/P95 latency 与错误率。
- **交互**：用模拟请求选择 purpose、数据分类、是否工具/视觉/结构化输出、推理策略，预览候选顺序；不产生真实模型调用。
- **异常**：registry reload 失败时继续显示上一个活动版本，并明确标记“数据库草稿未生效”。
...
flowchart LR
  ...
  Validate --> Draft["POST 保存配置草稿"]
  Draft --> Probe["POST 连接/能力测试"]
  Probe --> Publish["POST 发布配置版本"]
